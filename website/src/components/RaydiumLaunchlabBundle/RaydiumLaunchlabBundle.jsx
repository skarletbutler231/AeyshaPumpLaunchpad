...
import { FaDatabase, FaEllipsisV, FaExclamationTriangle, FaQuestion, FaRegCopy, FaSave, FaRedo, FaMars, FaRandom, FaPen, FaTools, FaPlus, FaDownload } from "react-icons/fa";
...
import { ellipsisAddress, getTokenAmounts, isValidAddress, numberWithCommas } from "../../utils/methods";
...
export default function RaydiumLaunchlabBundle({ className }) {
    const {
        SERVER_URL,
        setLoadingPrompt,
        setOpenLoading,
    ...
    const [solAmount, setSolAmount] = useState("85");
    ...
    const [snipeMode, setSnipeMode] = useState(0);
    ...
    const handleOKNewWallets = async (walletCount, fresh) => {
        ...
        try {
            ...
            const newCurrentProject = {
                ...
                mirrorWallets: data.project.mirrorWallets
            };
    ...
    const handleSimulate = async () => {
        ...
        if (useExpertMode) {
            ...
            console.log(snipePercent, walletCount)
            const walletAmounts = getTokenAmounts(MAX_AMOUNT_ON_CURVE / 100 * Number(snipePercent), Number(walletCount));
            console.log(walletAmounts)
            let newWalletChecked = []
            let newWalletAmount = []
            let newWalletSolAmount = []
            for (let i = 0; i < currentProject.wallets.length; i++) {
                if (i >= Number(walletCount)) {
                    newWalletChecked.push(false)
                    newWalletAmount.push("");
                    newWalletSolAmount.push("");
                    continue;
                }
        ...
        try {
            ...
            setOpenLoading(true);
            await axios.post(`${SERVER_URL}/api/v1/project/raydium-launchlab-simulate`,
                {
                    projectId: currentProject._id,
                    token,
                    tokenAmount,
    ...
    const handleDisperseSOL = async () => {
        ...
        try {
            ...
            await axios.post(`${SERVER_URL}/api/v1/project/launchlab-disperse`,
                {
                    projectId: currentProject._id,
                    simulateData,
                    signingData,
    ...
    const handleBuyTokens = async () => {
        ...
        try {
            setLoadingPrompt("Mint and Sniping LaunchLab Tokens...");
            setOpenLoading(true);

            await axios.post(`${SERVER_URL}/api/v1/project/buy-launchlab`,
                {
                    projectId: currentProject._id,
                    simulateData,
                    signingData,
            ...
            <div className="w-full h-full flex flex-col">
                ...
                {useExpertMode ?
                    <>
                        ...
                        <div className="flex justify-between items-center w-full gap-2 my-3 font-sans">
                        ...
                        <div className="w-full h-[30%] grow overflow-visible font-sans">
                            <div className="flex flex-col w-full h-full text-white bg-transparent bg-clip-border">
                                <div className="w-full h-full relative border border-gray-highlight rounded-lg">
                                    <div className={`min-h-[300px] h-full overflow-y-auto`}>
