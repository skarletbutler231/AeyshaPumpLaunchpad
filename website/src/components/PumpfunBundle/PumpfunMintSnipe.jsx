...
export default function PumpfunMintSnipe({ className }) {
    const {
        SERVER_URL,
        setLoadingPrompt,
        setOpenLoading,
    ...
    useEffect(() => {
        ...
        else if (notifyStatus.tag === "MINT_SNIPE_COMPLETED") {
            if (notifyStatus.success)
                toast.success("Succeed to mint and snipe pump fun token!");
            else
                toast.warn("Failed to snipe while succeeded mint!");
