...
export default function PumpfunSimulationDialog({ isOpen, zombie, tokenInfo, buyTokenAmount, tokenAmountInLP, solAmountInLP, onClose, handleDownloadSimuation, handleSimulateAgain, handleBundle, handleDisperseSOL, simulateData, alreadyMint }) {
    const { solInfo } = useContext(AppContext);
    const [copied, setCopied] = useState(false);
    const [solSum, setSolSum] = useState(0);
    ...
    useEffect(() => {
        let solSum1 = 0;
        simulateData?.wallets?.map((w) => {
            if (w.sim.buy.solAmount != "")
                solSum1 += w.sim.buy.solAmount;
        })
        setSolSum(Number(solSum1) / LAMPORTS_PER_SOL);
    }, [simulateData])
            ...
            <div className="flex flex-col p-6 w-[550px] font-sans">
                ...
                <div className="flex flex-col w-full h-auto  md:py-0 rounded-b-md">
                    ...
                    <div className="mt-3">
                        ...
                        <div className="w-full rounded-[10px] border-solid border border-[#2D3350] bg-[#111215] font-conthrax p-2 text-xs text-left font-bold text-yellow-normal">{((30 + solSum) / (1073000000 - buyTokenAmount) * (solInfo ? solInfo.price : 1)).toFixed(12)} {solInfo ? "USD" : "SOL"}</div>
                    </div>
                    <div className="mt-3">
                        <div className="flex justify-between gap-4">
                            <label className="my-1 text-sm text-white">
                        ...
                        <div className="w-full rounded-[10px] border-solid border border-[#2D3350] bg-[#111215] font-conthrax p-2 text-xs text-left font-bold text-yellow-normal">{formatNumber((30 + solSum) / (1073000000 - buyTokenAmount) * (solInfo ? solInfo.price : 1) * 1000000000)} {solInfo ? "USD" : "SOL"}</div>
                    </div>
                    ...
                    <div className="mt-2 w-full">
                        <div className={`${alreadyMint ? "bg-gray-dark" : "bg-gradient-blue-to-purple"} rounded-md p-[1px]`}>
                            <ExtendedButton className={`bg-black/50 gap-2 !h-10 w-full font-conthrax uppercase text-xxs`} onClick={handleBundle} disabled={alreadyMint}>
                                Mint & Buy
