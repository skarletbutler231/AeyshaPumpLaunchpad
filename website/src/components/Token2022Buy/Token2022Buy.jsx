...
import { createPool, getTipTransaction, makeFreezeAccountTransaction, sendAndConfirmSignedTransactions, setFreezeAuthority } from "../../utils/solana";
import { ellipsisAddress, getTokenAmounts, isValidAddress } from "../../utils/methods";
import BundleProgressDialog from "../Dialogs/BundleProgressDailog";
import { createCpmmPool } from "../../utils/raydiumSdk";

export default function Token2022Buy({ className }) {
    const {
        SERVER_URL,
        setLoadingPrompt,
        setOpenLoading,
    ...
    useEffect(() => {
        const updateBalance = async (connection, tokenAddress, owner) => {
            ...
            try {
                const mint = new PublicKey(tokenAddress);
                const mintInfo = await getMint(connection, mint, undefined, TOKEN_2022_PROGRAM_ID);
                const tokenATA = await getAssociatedTokenAddress(mint, owner, undefined, TOKEN_2022_PROGRAM_ID);
                const tokenAccountInfo = await getAccount(connection, tokenATA, undefined, TOKEN_2022_PROGRAM_ID);
    ...
    const handleSimulate = async () => {
        ...
        try {
            setLoadingPrompt("Simulating...");
            setOpenLoading(true);
            await axios.post(`${SERVER_URL}/api/v1/project/simulate-cpmm-fair`,
                {
                    projectId: currentProject._id,
                    token,
                    tokenAmount,
    ...
    const handleBuyTokens = async () => {
        ...
        try {
            ...
            const { transaction: createPoolTransaction, poolInfo } = await createCpmmPool(
                connection,
            ...
            const freezePoolTransaction = await makeFreezeAccountTransaction(connection, token, publicKey, new PublicKey(poolInfo.vaultB), TOKEN_2022_PROGRAM_ID);
            const setAuthorityTransaction = await setFreezeAuthority(connection, token, publicKey, zombieWallet.address, TOKEN_2022_PROGRAM_ID)
            const tipTransaction = await getTipTransaction(connection, publicKey, user.presets.jitoTip);
            let transactions = [createPoolTransaction, freezePoolTransaction, setAuthorityTransaction, tipTransaction];

            setLoadingPrompt("Signing...");
            const signedTxns = await signAllTransactions(transactions);

            setLoadingPrompt("Creating pool and Buying Tokens...");
            const ret = await sendAndConfirmSignedTransactions(true, connection, signedTxns, signingData, sigData);

            if (ret) {
                await axios.post(`${SERVER_URL}/api/v1/project/buy-cpmm-fair`,
                    {
                        projectId: currentProject._id,
                        poolInfo,
                        simulateData,
